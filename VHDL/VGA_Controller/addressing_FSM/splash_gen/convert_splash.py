...
def convert(filename):
    ...
    for line in lines[:DIMENSION[1]]:
        ...
        for i in range(0, min(DIMENSION[0]*2, len(conv)), 4):
            arr.append(conv[i:i+4])
    arr.extend(['2020'] * ((DIMENSION[0] * DIMENSION[1] / 2) - len(arr)))
    data = "\n".join(arr)
    open("memory.txt", "wb").write(data)
    data = "\n".join(' '.join('x"%s",' %a for a in arr[i:i+20]) for i in range(0, len(arr), 20))
